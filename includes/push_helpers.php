...
function push_notify_new_ticket(
    PDO $pdo,
    int $ticketId,
    string $trackingCode,
    string $ticketTitle
...
{
    $code = trim($trackingCode) !== '' ? $trackingCode : (string)$ticketId;
    $subject = trim($ticketTitle) !== '' ? $ticketTitle : 'بدون عنوان';
    $title = 'تیکت جدید';
    $body = 'تیکت ' . $code . ' — ' . $subject;
    $url = '/admin/view-ticket.php?id=' . $ticketId;

    push_notify_all_admins($pdo, $title, $body, $url, 'ticket-new-' . $ticketId);
}
